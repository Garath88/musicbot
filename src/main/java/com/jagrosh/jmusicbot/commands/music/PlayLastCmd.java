 ...
 */
public class PlayLastCmd extends MusicCommand {
    private final static String LOAD = "\uD83D\uDCE5"; // 📥
    private final static String CANCEL = "\uD83D\uDEAB"; // 🚫

    private final String loadingEmoji;

    public PlayLastCmd(Bot bot, String loadingEmoji) {
        super(bot);
        this.loadingEmoji = loadingEmoji;
        this.name = "playlast";
        this.arguments = "<title|URL|subcommand>";
        this.help = "add the provided song at the end of the queue";
    ...
    private class ResultHandler implements AudioLoadResultHandler {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist) {
            ...
            final String addMsg;
            if (pos == 0) {
                trackMsg += "to begin playing";
            } else {
                trackMsg += "to the queue at position " + pos;
            }
            if (pos == -1) {
                addMsg = "Duplicate track not added";
            } else {
                addMsg = FormatUtil.filter(event.getClient().getSuccess() + " Added **" + track.getInfo().title
                    + "** (`" + FormatUtil.formatTime(track.getDuration()) + "`) " + trackMsg);
            }
